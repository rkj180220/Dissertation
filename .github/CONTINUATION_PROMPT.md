...
> **Run #6 complete. P14 fixed. Run #7 needed to verify correct DB/Cache sizing. P15 is the next major feature phase — expanded to 9 items including Router Agent, Validator Agent, and Session Persistence.**
...
- **P15 Router/Validator/Session design (8 May 2026)**: Three new agents designed. (1) **Router Agent** (`src/agents/router.py`): LLM-based intent classifier at the entry point for turn≥2 requests. Reads OrchestratorState + new user input → classifies: `new_request|amendment|validate|answer|clarify`. For amendment: skips Clarifier, runs Profiler(delta)→Sizer→FinOps→Validator→RFP Writer with changelog section. For validate: routes directly to Validator. For answer: router writes response inline. (2) **Validator Agent** (`src/agents/validator.py`): architecture scoring (serverless=0.895 vs containers=0.670 for 2M concurrent users) + sizing adequacy check + budget fit + WAF compliance. Automatically inserted between FinOps and RFP Writer in every pipeline run. Writes `validation_report` to state. (3) **Session Persistence**: LangGraph `SqliteSaver` checkpointer from `langgraph-checkpoint-sqlite` package. Thread ID = `session_id` (UUID from client). `graph.compile(checkpointer=checkpointer)` — LangGraph natively restores prior state. `OrchestrationRequest` gets `session_id: str | None`; `OrchestrationResponse` returns `session_id`. CRITICAL: current code deletes `_clarify_sessions[request_id]` on `status=ready` — must NOT do this; session must persist. Entry conditional: `if state.get("rfp_document") → router else → clarifier`. Full scoring formula in PROJECT_SPEC §22f.
- **P15 gap analysis (7 May 2026)**: Presidio RFP comparison revealed 11 structural gaps. The biggest: (1) We never consider serverless — for 50K→2M concurrent users, Lambda+DynamoDB is objectively better (scales to zero, unlimited concurrency, cost ∝ requests). Our system hardcodes containers/K8s because the clarifier LLM says "Kubernetes" in enriched_input and the profiler blindly maps to CONTAINER. Fix: add `architecture_selector.py` engine that scores alternatives BEFORE profiling workloads. (2) Profiler produces 1 container workload for all of Cal Fire's backend — all 6 microservices collapse to one CONTAINER entry. Bin-packing is correct but pointless with 1 workload. Fix: update profiler LLM prompt to explicitly decompose into named microservices. (3) Incremental amendments: every follow-up message triggers full pipeline restart. Fix: Router Agent detects amendment intent and routes to a shortened graph path.
